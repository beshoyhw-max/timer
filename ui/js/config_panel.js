...
(function () {
    ...
    // Animation Messages
    const inputTextOnTime = document.getElementById('input-text-on-time');
    const inputTextOvertime = document.getElementById('input-text-overtime');
    ...
    function renderFontOptions(fonts) {
        ...
        if (fonts.length === 0) {
            ...
            msg.textContent = I18N.t('feedback.noFonts');
    ...
    function addTierRow(tier = { threshold_mins: 0, rate_amount: 5, rate_interval: 5, unit: 'sec' }, index = -1) {
        ...
        row.innerHTML = `
      <span class="tier-label">${I18N.t('tier.after')}</span>
      ...
      <span class="tier-label">${I18N.t('tier.minArrow')}</span>
      ...
      <select class="narrow tier-unit" title="Billing Unit" style="width: 50px;">
        <option value="sec" ${!isMin ? 'selected' : ''}>${I18N.t('tier.secOption')}</option>
        <option value="min" ${isMin ? 'selected' : ''}>${I18N.t('tier.minOption')}</option>
      </select>
      <span class="interval-group ${isMin ? 'hidden' : ''}" style="display: inline-flex; align-items: center;">
        ...
        <span class="tier-label">${I18N.t('tier.secUnit')}</span>
      </span>
    ...
    function loadAppearance(app) {
        ...
        if (app.text_on_time) inputTextOnTime.value = app.text_on_time;
        if (app.text_overtime) inputTextOvertime.value = app.text_overtime;
    ...
    function collectAppearance() {
        return {
            ...
            text_on_time: inputTextOnTime.value || '准时完成!',
            text_overtime: inputTextOvertime.value || '议题超时，请赞助',
        };
    }
    ...
    // Auto-save on text message changes
    inputTextOnTime.addEventListener('input', saveAppearance);
    inputTextOvertime.addEventListener('input', saveAppearance);
    ...
    btnBgImage.addEventListener('click', async () => {
        try {
            ...
            if (filename) {
                ...
                btnBgImage.textContent = I18N.t('feedback.imageChanged');
                setTimeout(() => { btnBgImage.textContent = I18N.t('btn.changeImage'); }, 1500);
            }
        } catch (e) {
