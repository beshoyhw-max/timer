...
let _cachedAppearance = {};

function applyAppearance(app) {
  if (!app) return;
  _cachedAppearance = app;
  ...
  if (app.font_family) {
    ...
    const tyCost = document.getElementById('ty-cost');
    const tyText = document.getElementById('ty-text');
    const animQuote = document.getElementById('animation-quote');
    if (clock) clock.style.fontFamily = fontStack;
    if (moneyCounter) moneyCounter.style.fontFamily = fontStack;
    if (tyCost) tyCost.style.fontFamily = fontStack;
    if (tyText) tyText.style.fontFamily = fontStack;
    if (animQuote) animQuote.style.fontFamily = fontStack;
  }
}
...
function renderState(state) {
  ...
  // Phase class on floating timer (preserve hover state)
  const isHovered = DOM.timer.classList.contains('hovered');
  DOM.timer.className = isHovered ? 'hovered' : '';
  if (phase === 'idle' || phase === 'collecting' || phase === 'thankyou') {
    if (phase === 'idle') DOM.timer.classList.add('idle');
    else DOM.timer.classList.add(`phase-${phase}`);
  ...
  if (phase === 'overtime' || (phase === 'paused' && state.overtime_seconds > 0)) {
    ...
    DOM.clock.textContent = `${sign}${String(otMin).padStart(2, '0')}:${String(otSec).padStart(2, '0')}`;
  ...
  const showMoney = phase === 'overtime' || (phase === 'paused' && state.overtime_seconds > 0);
  if (showMoney) {
  ...
  // Toggle class on bar-center so CSS can adjust clock size
  const barCenter = document.getElementById('bar-center');
  if (barCenter) {
    barCenter.classList.toggle('money-visible', showMoney);
  }
  ...
  // Auto-scale content to fit within the window
  fitBarCenter();
}

/* ═══════ AUTO-SCALE BAR CONTENT ═══════ */

function fitBarCenter() {
  const barCenter = document.getElementById('bar-center');
  if (!barCenter) return;

  // Reset transform to measure natural size
  barCenter.style.transform = 'none';
  void barCenter.offsetWidth; // force reflow

  // Available width = barCenter's allocated flex space
  const availableW = barCenter.clientWidth;

  // Measure natural content span (first child left → last child right)
  const visible = [...barCenter.children].filter(c =>
    !c.classList.contains('hidden') && window.getComputedStyle(c).display !== 'none'
  );

  if (visible.length === 0) {
    barCenter.style.transform = '';
    return;
  }

  const firstRect = visible[0].getBoundingClientRect();
  const lastRect = visible[visible.length - 1].getBoundingClientRect();
  const contentW = lastRect.right - firstRect.left;

  // Apply scale if content overflows
  if (contentW > availableW && availableW > 0) {
    const scale = Math.max(0.3, availableW / contentW);
    barCenter.style.transform = `scale(${scale})`;
  } else {
    barCenter.style.transform = '';
  }
}
...
async function playEnvelopeAnimation(cost, quote) {
  ...
  try {
    ...
    document.getElementById('ty-text').textContent = _cachedAppearance.text_overtime || '议题超时，请赞助';
...
async function playOnTimeAnimation(speaker, quote) {
  ...
  try {
    ...
    document.getElementById('ty-text').textContent = _cachedAppearance.text_on_time || '准时完成!';
...
/* ═══════ HOVER DETECTION (JS-driven for frameless window) ═══════ */

(function () {
  const ft = document.getElementById('floating-timer');
  if (!ft) return;

  ft.addEventListener('mouseenter', () => ft.classList.add('hovered'));
  ft.addEventListener('mouseleave', () => ft.classList.remove('hovered'));

  // Failsafe: periodically check if cursor is still inside the window
  // Frameless windows sometimes miss the mouseleave event at edges
  let lastX = -1, lastY = -1;
  document.addEventListener('mousemove', (e) => { lastX = e.clientX; lastY = e.clientY; });

  setInterval(() => {
    if (!ft.classList.contains('hovered')) return;
    const w = window.innerWidth;
    const h = window.innerHeight;
    if (lastX <= 0 || lastY <= 0 || lastX >= w - 1 || lastY >= h - 1) {
      ft.classList.remove('hovered');
    }
  }, 200);
})();
...
if (resizeGrip) {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    const dw = e.screenX - startScreenX;
    const dh = e.screenY - startScreenY;

    if (dw !== 0 || dh !== 0) {
      pywebview.api.resize_window(dw, dh).catch(() => { });
      startScreenX = e.screenX;
      startScreenY = e.screenY;
    }
  });
