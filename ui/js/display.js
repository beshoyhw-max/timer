...
function renderState(state) {
  ...
  if (phase === 'overtime') {
    ...
    DOM.moneyValue.textContent = `￥${state.cost.toFixed(2)}`;
  } else {
  ...
  // ═══════ ALARM TRIGGERS ═══════
  if (state.alarm_1) {
    AlarmSounds.playAlarm('warning1');
  }
  if (state.alarm_2) {
    AlarmSounds.playAlarm('warning2');
  }
...
async function playEnvelopeAnimation(cost, quote) {
  ...
  DOM.envelopeCost.textContent = `￥${cost.toFixed(2)}`;
  ...
  DOM.tyCost.textContent = `￥${cost.toFixed(2)}`;
  await sleep(200);
...
async function playOnTimeAnimation(speaker, quote) {
  ...
  DOM.tyCost.textContent = '✅ ￥0.00';
  DOM.tyCost.style.color = 'var(--teal)';
